...
export default function Ranking() {
  ...
  // Paleta de colores para ranking - adaptada al estilo minimalista
  ...
  // Tarjeta de usuario actual - estilo minimalista
  const UserRankCard = ({ userRank, userData }) => {
    if (!userRank || !userData) return null;

    return (
      <div className="bg-black/40 border border-fuchsia-500/30 rounded-lg p-4 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="text-2xl">⭐</div>
            <div>
              <h3 className="text-white font-bold text-sm">Tu Posición</h3>
              <p className="text-gray-300 text-xs">
                Puesto #{userRank} de {filteredUserScores.length}
              </p>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-gray-300 text-xs">
              {calculateTotalAccuracy(userData)}% precisión
            </div>
          </div>
        </div>
        ...
        <div className="mt-3">
          ...
          <div className="w-full bg-gray-700 rounded-full h-1.5">
            <div 
              className="bg-gradient-to-r from-fuchsia-500 to-purple-500 h-1.5 rounded-full transition-all duration-1000"
  ...
  const Podium = ({ topThree }) => (
    <div className="grid grid-cols-3 gap-3 mb-6 items-end">
      ...
      <div className="transform hover:scale-105 transition-transform">
        <div className="bg-gradient-to-b from-gray-400 to-gray-500 h-16 rounded-t-lg flex items-center justify-center relative border border-gray-300">
          <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-lg">
            🥈
          </div>
          <div className="text-gray-800 text-xs font-bold">#2</div>
        </div>
        <div className="bg-gray-600 p-2 rounded-b-lg border border-gray-500">
          <div className="text-white font-bold text-xs truncate text-center">
            {topThree[1]?.userName?.split(' ')[0] || '-'}
          </div>
          <div className="text-amber-300 text-sm font-bold text-center">
      ...
      {/* Primer lugar */}
      <div className="transform hover:scale-105 transition-transform">
        <div className="bg-gradient-to-b from-amber-300 to-amber-400 h-20 rounded-t-lg flex items-center justify-center relative border border-amber-200">
          <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 text-xl">
            💎
          </div>
          <div className="text-amber-900 text-xs font-bold">#1</div>
        </div>
        <div className="bg-amber-600 p-2 rounded-b-lg border border-amber-500">
          <div className="text-white font-bold text-xs truncate text-center">
      ...
      {/* Tercer lugar */}
      <div className="transform hover:scale-105 transition-transform">
        <div className="bg-gradient-to-b from-amber-600 to-amber-700 h-14 rounded-t-lg flex items-center justify-center relative border border-amber-500">
          <div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
            🥉
          </div>
          <div className="text-amber-100 text-xs font-bold">#3</div>
        </div>
        <div className="bg-amber-700 p-2 rounded-b-lg border border-amber-600">
          <div className="text-white font-bold text-xs truncate text-center">
  ...
  const RankingItem = ({ user, index, isCurrentUser }) => {
    ...
    return (
      <div className={`bg-black/30 border ${isCurrentUser ? 'border-amber-400' : 'border-gray-600'} rounded-lg p-3 transition-all duration-300 hover:border-fuchsia-400`}>
        ...
        >
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            <div className="relative flex-shrink-0">
              <div className={`text-white text-xs font-bold ${badge.bgColor} rounded-full w-6 h-6 flex items-center justify-center`}>
                {index + 1}
              </div>
            </div>
          ...
          <div className="mt-3 border-t border-gray-600 pt-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(user.scoresByPhase || {}).map(([phase, points]) => {
                ...
                return (
                  <div key={phase} className="bg-black/20 rounded p-2 border border-gray-600">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center space-x-1">
                        <span className="text-xs">{phaseInfo.icon}</span>
        ...
        <div className="text-center mt-2">
          <button 
            onClick={() => setExpandedUser(isExpanded ? null : user.userId)}
            className="text-gray-400 hover:text-white text-xs transition-colors duration-200 bg-black/20 px-2 py-1 rounded"
          >
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-white text-lg">Calculando ranking...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-fuchsia-400 mb-2">
            Ranking de Missologas
          </h1>
          <p className="text-gray-300 text-sm sm:text-base">
            {filteredUserScores.length} usuarios compitiendo
          </p>
        </div>
        ...
        {topThree.length > 0 && (
          <div className="mb-6">
            <h2 className="text-xl font-bold text-center text-white mb-3 text-sm sm:text-base">
              Top 3
            </h2>
        ...
        {filteredUserScores.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl text-white mb-3">¡No hay usuarios activos!</h3>
            <p className="text-gray-400 text-sm mb-6">Los resultados aparecerán cuando los usuarios hagan predicciones</p>
            <button 
              onClick={() => navigate('/predict')}
        ...
        <div className="mt-8 text-center">
          <div className="bg-black/40 border border-fuchsia-500/30 rounded-lg p-4 max-w-2xl mx-auto">
            <h4 className="text-fuchsia-400 text-lg font-bold mb-3 text-sm sm:text-base">Sistema de Puntuación</h4>
            ...
            <p className="text-gray-300 text-xs sm:text-sm">
              Total máximo: <span className="text-amber-400 font-bold">100 puntos</span> 
            </p>
          </div>
        </div>
      </div>
